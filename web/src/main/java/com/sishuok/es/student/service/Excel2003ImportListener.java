...
class Excel2003ImportListener implements HSSFListener {
    ...
    Excel2003ImportListener(
            final StudentService studentService, final List<Student> dataList, final int batchSize) {
            ...
            case LabelSSTRecord.sid:
                if(record instanceof NumberRecord) {
                    ...
                    if(lrec.getRow() == 0) {
                        ...
                        current.setSno(value);
                    } else if(lrec.getColumn() == 1) {//第二列
                        String value = sstrec.getString(lrec.getSSTIndex()).getString();
                        current.setName(value);
                        add(current);
                    } else if(lrec.getColumn() == 2) {//第三列
                        String value = sstrec.getString(lrec.getSSTIndex()).getString();
    ...
    private void add(final Student current) {
        ...
        if (totalSize % batchSize == 0) {
            studentService.doBatchSave(dataList);
            dataList.clear();
        }
    }
}
